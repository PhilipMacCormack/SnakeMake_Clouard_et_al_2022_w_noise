...
from Scripts.Scripts_pooling.pooler import *
from Scripts.Scripts_pooling import pybcf
from Scripts.Scripts_pooling import utils
...
np.random.seed(1)  # fix the shuffling order
...
class ShuffleSplitVCF(object):
    ...
    """
    def __init__(self, design_matrix: np.ndarray, vcf_in: str, stu_size: float = 0.1, wd: str = os.getcwd(), output_path = 'Output/splitting/'):
        """
        ...
        * approximately 1/10 of the reference panel
        ...
        self.ref_pan = os.path.join(wd, output_path + 'reference.panel')
        self.stu_pop = os.path.join(wd, output_path + 'study.population')
        self.ref_prefix = output_path + 'REF'
        self.stu_prefix = output_path + 'IMP'
        self.wd = wd
...
class VariantRecordPooler(object):
    ...
    """
    def __init__(self, design_matrix: np.ndarray, noise: str, noise_intensity: float, var: pysam.VariantRecord,
                 dict_lookup: dict, format_to: str, call_freq_v = '', wd: str = os.getcwd()):
        """
    ...
    def _decode(self) -> np.ndarray:
        ...
        self.varp = self._encode()
        if self.noise == True:
            self._add_noise(self.varp)
        return dict_blocks_decoder(self.n_blocks, self.varp.sum(axis=-1), y_shift, self.lookup, self.fmt_to.lower())

    def _add_noise(self,enc_mat):
        import random as rand
        call_freq_data = pd.read_csv(self.call_freq_v, sep='\t')
        pos = self.var.pos
        call_freq = call_freq_data.loc[call_freq_data.Position == int(pos), 'Call_Freq'].values[0]
        noise_SNPs = open("noise_SNPs.txt", "a")
        no_spaces = 8-int(len(str(pos)))
        space_string = ' ' * no_spaces
        for genotype in enc_mat:
            for allele in genotype:
                r = rand.uniform(self.noise_intensity,1)
                if r >= call_freq:
                    if allele[0] == 0 and allele[1] == 0:
                        allele[0] = 1
                        if len(str(pos)) < 8:
                            noise_SNPs.write("\n" + f'{pos}' + space_string + "\t" + str([0,0]) + "\t" + str([1,0]))
                        else:
                            noise_SNPs.write("\n" + f'{pos}' + "\t" + str([0,0]) + "\t" + str([1,0]))
                    elif allele[0] == 1 and allele[1] == 0:
                        equal_prob_r = rand.uniform(0,1)
                        if equal_prob_r >= 0.5:
                            allele[0] = 0
                        else:
                            allele[1] = 1
                        if len(str(pos)) < 8:
                            noise_SNPs.write("\n" + f'{pos}' + space_string + "\t" + str([0,1]) + "\t" + str([0,0]))
                        else:
                            noise_SNPs.write("\n" + f'{pos}' + "\t" + str([0,1]) + "\t" + str([0,0]))
                    elif allele[0] == 1 and allele[1] == 1:
                        allele[1] = 0
                        if len(str(pos)) < 8:
                            noise_SNPs.write("\n" + f'{pos}' + space_string + "\t" + str([1,1]) + "\t" + str([1,0]))
                        else:
                            noise_SNPs.write("\n" + f'{pos}' + "\t" + str([1,1]) + "\t" + str([1,0]))


    def new_var(self, noise) -> str:
        """
        ...
        if self.fmt_to == 'GP':
            _genotypes = self._decode().reshape((self.genotypes.shape[0], 3))  # 3 --> GP(RR, RA, AA)
            equal_genotypes = [-0.4771]*48
            allele_check = []
            for lista in _genotypes:
                for allele in lista:
                    allele_check.append(allele)
                    if len(allele_check) == 48:
                        j = allele_check
                        allele_check = []
                        if np.array_equal(j, equal_genotypes):
                            incompatible_genotypes = open("incompatible_genotypes.txt", "a")
                            incompatible_genotypes.write("\n")
                            incompatible_genotypes.write("{}\t{}\t{}\nPOOL GENOTYPES:\n{}\n".format(self.var.chrom,self.var.pos,self.var.id,self.varp.reshape(16,15)))
...
class VariantFilePooler(object):
    ...
    """
    def __init__(self, design_matrix: np.ndarray, noise: str, noise_intensity: float, vcf_in: str, vcf_out: str,
                 dict_lookup: dict, format_to: str, call_freq_v, wd: str = os.getcwd()):
        """
        The NonOverlapping Repeated Block pooling design applied is provided with the design matrix.
...
#       import random as rand
#       call_freq_data = pd.read_csv(self.call_freq_v, sep='\t')
#       genotypes = np.asarray([v['GT'] for v in var.samples.values()])
#       pos = var.pos
#       call_freq = call_freq_data.loc[call_freq_data.Position == int(pos), 'Call_Freq'].values[0]       
#       noise_SNPs = open("noise_SNPs.txt", "a")
#       no_spaces = 8-int(len(str(pos)))
#       space_string = ' ' * no_spaces
#       for genotype in var.samples.values():
#           r = rand.uniform(self.noise_intensity,1)
#           if r >= call_freq:
#               if genotype['GT'][0] == 0 and genotype['GT'][1] == 0:
#                   genotype['GT'] = (1,0)
#                   if len(str(pos)) < 8:                        
#                       noise_SNPs.write("\n" + f'{pos}' + space_string + "\t" + str([0,0]) + "\t" + str([1,0]))
#                   else:
#                       noise_SNPs.write("\n" + f'{pos}' + "\t" + str([0,0]) + "\t" + str([1,0]))
#               elif genotype['GT'][0] == 1 and genotype['GT'][1] == 0 or genotype['GT'][0] == 0 and genotype['GT'][1] == 1 :
#                   equal_prob_r = rand.uniform(0,1)
#                   if equal_prob_r >= 0.5:
#                       genotype['GT'] = (0,0)
#                   else:
...
#                   else:
#                       noise_SNPs.write("\n" + f'{pos}' + "\t" + str([0,1]) + "\t" + str([0,0]))
#               elif genotype['GT'][0] == 1 and genotype['GT'][1] == 1:
#                   genotype['GT'] = (1,0)
#                   if len(str(pos)) < 8:
#                       noise_SNPs.write("\n" + f'{pos}' + space_string + "\t" + str([1,1]) + "\t" + str([1,0]))
#                   else:
#                       noise_SNPs.write("\n" + f'{pos}' + "\t" + str([1,1]) + "\t" + str([1,0]))
    ...
    def _new_data(self):
        ...
        for n, var in enumerate(self.vcf_in.fetch()):
            #if self.noise == True:
            #    self._add_noise(var)
            prec = VariantRecordPooler(self.design,self.noise, self.noise_intensity, var, self.lookup, self.fmt_to, self.call_freq_v)
            pvar.append(prec.new_var(self))
            self.n_variants += 1
            if n % 1000 == 0:
...
def pysam_pooler_gp(noise: str, noise_intensity: float, file_in: str, file_out: str, path_to_lookup: str, wd: str, call_freq_v=''):
    """
    ...
    poolf = VariantFilePooler(dm, noise, noise_intensity,
...
def pysam_pooler_gt(noise: str, noise_intensity: float, file_in: str, file_out: str, path_to_lookup: str, wd: str, call_freq_v=''):
    """
    Process a VCF file with NORB pooling simulation. Decode into GP based on a look up table.
    :param file_in: name of the file to be processed (.vcf.gz or .vcf only)
    :param file_out: name of the file to output (NO .gz)
