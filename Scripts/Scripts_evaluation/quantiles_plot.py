...
from Scripts.Scripts_evaluation import quality as qual
from Scripts.Scripts_util.files import *
...
if compute:
    for metric, d in metrics.items():
        if d is not None:
            ...
            pctY_comp.to_json(jsonf, orient='records')
...
for dquant, f in dataquants.items():
    ...
    if f is not None:
        ...
        print(dataf)
        meanf = {}
        gY = sns.lineplot(data=dataf[dataf.quantile(0.5)], x=x_data, y=dquant,
                          hue='dataset', palette="deep", linewidth=1)

        for i, dset in enumerate(['beagle']):
            df = dataf[dataf['dataset'] == dset]
            meanf[dset] = df['mean'].mean()
            gY.fill_between(df[df.quantile(1.0)][x_data],
                            df[df.quantile(0.0)][dquant],
                            df[df.quantile(1.0)][dquant],
                            color=sns.color_palette('deep')[i],
                            alpha=0.1)
            gY.fill_between(df[df.quantile(0.99)][x_data],
                            df[df.quantile(0.01)][dquant],
                            df[df.quantile(0.99)][dquant],
                            color=sns.color_palette('deep')[i],
                            alpha=0.25)
            gY.fill_between(df[df.quantile(0.75)][x_data],
                            df[df.quantile(0.25)][dquant],
                            df[df.quantile(0.75)][dquant],
                            color=sns.color_palette('deep')[i],
                            alpha=0.40)
